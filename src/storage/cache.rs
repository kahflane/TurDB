...
mod tests {
    ...
    #[test]
    fn page_key_equality() {
        let k1 = PageKey::new(1, 10);
        let k2 = PageKey::new(1, 10);
        let k3 = PageKey::new(1, 11);
    ...
    #[test]
    fn cache_entry_visited_flag() {
        let entry = CacheEntry::new(PageKey::new(1, 0));

        assert!(!entry.clear_visited());
    ...
    #[test]
    fn cache_shard_insert_and_get() {
        let mut shard = CacheShard::new(10);
        let key = PageKey::new(1, 5);

        assert!(shard.get(&key).is_none());

        let entry = CacheEntry::new(key);
        let idx = shard.insert(entry);

        assert_eq!(shard.get(&key), Some(idx));
        assert_eq!(shard.len(), 1);
    }

    #[test]
    fn cache_shard_evict_unvisited() {
        let mut shard = CacheShard::new(3);

        let k1 = PageKey::new(1, 1);
        let k2 = PageKey::new(1, 2);
        let k3 = PageKey::new(1, 3);

        shard.insert(CacheEntry::new(k1));
        shard.insert(CacheEntry::new(k2));
        shard.insert(CacheEntry::new(k3));

        let (evicted, _) = shard.evict().unwrap();
        assert_eq!(evicted, k1);
    }

    #[test]
    fn cache_shard_evict_skips_visited() {
        let mut shard = CacheShard::new(3);
    ...
    #[test]
    fn cache_shard_evict_skips_pinned() {
        let mut shard = CacheShard::new(3);
    ...
    #[test]
    fn cache_shard_evict_returns_none_when_all_pinned() {
        ...
        assert!(shard.evict().is_none());
    }

    #[test]
    fn cache_shard_remove() {
        let mut shard = CacheShard::new(3);
        ...
        assert!(shard.get(&k2).is_some() || shard.get(&k3).is_some());
        assert_eq!(shard.len(), 2);
    }

    #[test]
    fn page_cache_new() {
        let cache = PageCache::new(128).unwrap();
    ...
    #[test]
    fn page_cache_new_minimum_capacity() {
        let result = PageCache::new(32);

        assert!(result.is_err());
    }

    #[test]
    fn page_cache_shard_distribution() {
        let cache = PageCache::new(128).unwrap();

        let s1 = cache.shard_index(&PageKey::new(0, 0));
        let s2 = cache.shard_index(&PageKey::new(0, 1));
        let s3 = cache.shard_index(&PageKey::new(1, 0));
    ...
    #[test]
    fn page_cache_get_or_insert() {
        let cache = PageCache::new(64).unwrap();
        let key = PageKey::new(1, 5);

        let page = cache
            .get_or_insert(key, |data| {
                data[0] = 0xAB;
                Ok(())
            })
            .unwrap();
    ...
    #[test]
    fn page_cache_get_after_insert() {
        let cache = PageCache::new(64).unwrap();
        let key = PageKey::new(1, 5);
    ...
    #[test]
    fn page_cache_auto_unpin_on_drop() {
        let cache = PageCache::new(64).unwrap();
        let key = PageKey::new(1, 5);

        {
            let _page = cache.get_or_insert(key, |_| Ok(())).unwrap();
        }

        let shard = cache.shard(&key);
        let guard = shard.read();
        let idx = guard.get(&key).unwrap();
        assert!(!guard.entries[idx].is_pinned());
    }

    #[test]
    fn page_cache_dirty_tracking() {
        let cache = PageCache::new(64).unwrap();
        let key = PageKey::new(1, 5);
    ...
    fn page_cache_flush_dirty() {
        ...
        cache.get_or_insert(k1, |_| Ok(())).unwrap();
        cache.get_or_insert(k2, |_| Ok(())).unwrap();
        cache.get_or_insert(k3, |_| Ok(())).unwrap();
        ...
        assert_eq!(count, 2);
        assert!(flushed_keys.contains(&k1));
        assert!(flushed_keys.contains(&k3));
        assert!(!flushed_keys.contains(&k2));

        assert!(!cache.is_dirty(&k1));
        assert!(!cache.is_dirty(&k3));
    }

    #[test]
    fn page_cache_eviction_on_full() {
        let cache = PageCache::new(64).unwrap();

        for i in 0..65 {
            let key = PageKey::new(0, i);
            cache.get_or_insert(key, |_| Ok(())).unwrap();
        }
    ...
    #[test]
    fn page_ref_data_mut_marks_dirty() {
        let cache = PageCache::new(64).unwrap();
        let key = PageKey::new(1, 5);

        {
            let mut page = cache.get_or_insert(key, |_| Ok(())).unwrap();
    ...
    fn sieve_gives_second_chance() {
        ...
        let (evicted, _) = shard.evict().unwrap();
        assert_eq!(evicted, k2, "k2 should be evicted - it was not accessed");

        shard.remove(shard.get(&k2).unwrap());

        assert!(
            shard.get(&k1).is_some(),
            "k1 should survive - it was accessed and got second chance"
        );
    }
}

...
mod tests {
    ...
    fn parse_between_expression() {
        let arena = Bump::new();
        let mut parser = Parser::new("SELECT * FROM t WHERE x BETWEEN 1 AND 10", &arena);
        let stmt = parser.parse_statement().unwrap();
        if let Statement::Select(select) = stmt {
            assert!(select.where_clause.is_some());
        } else {
    ...
    fn parse_in_subquery() {
        let arena = Bump::new();
        let mut parser = Parser::new("SELECT * FROM t WHERE id IN (SELECT user_id FROM orders)", &arena);
        let stmt = parser.parse_statement().unwrap();
        assert!(matches!(stmt, Statement::Select(_)));
    }
    ...
    fn parse_like_expression() {
        let arena = Bump::new();
        let mut parser = Parser::new("SELECT * FROM t WHERE name LIKE '%test%'", &arena);
        let stmt = parser.parse_statement().unwrap();
        assert!(matches!(stmt, Statement::Select(_)));
    }
    ...
    fn parse_offset_rows_fetch_first() {
        let arena = Bump::new();
        let mut parser = Parser::new("SELECT * FROM users OFFSET 5 ROWS FETCH FIRST 10 ROWS ONLY", &arena);
        let stmt = parser.parse_statement().unwrap();
        if let Statement::Select(select) = stmt {
            assert!(select.offset.is_some());
            assert!(select.limit.is_some());
    ...
    fn parse_grant_multiple_privileges() {
        let arena = Bump::new();
        let mut parser = Parser::new("GRANT SELECT, INSERT, UPDATE ON users TO admin, moderator", &arena);
        let stmt = parser.parse_statement().unwrap();
        if let Statement::Grant(grant) = stmt {

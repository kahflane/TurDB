...
mod tests {
    ...
    #[test]
    fn datatype_discriminants_are_stable() {
        assert_eq!(DataType::Bool as u8, 0);
        assert_eq!(DataType::Int2 as u8, 1);
        assert_eq!(DataType::Int4 as u8, 2);
        assert_eq!(DataType::Int8 as u8, 3);
        assert_eq!(DataType::Float4 as u8, 4);
        assert_eq!(DataType::Float8 as u8, 5);
        assert_eq!(DataType::Text as u8, 20);
        assert_eq!(DataType::Vector as u8, 22);
        assert_eq!(DataType::Jsonb as u8, 23);
        assert_eq!(DataType::Array as u8, 71);
    }

    #[test]
    fn datatype_try_from_roundtrip() {
        for dt in [
            DataType::Bool,
            DataType::Int8,
            DataType::Text,
            ...
            let byte = dt as u8;
            let recovered = DataType::try_from(byte).unwrap();
    ...
    fn variable_size_types() {
        ...
        assert!(DataType::Array.is_variable());
        assert!(!DataType::Int8.is_variable());
    }
    ...
    fn type_affinity() {
        ...
        assert_eq!(DataType::Uuid.affinity(), TypeAffinity::Numeric);
    }
}

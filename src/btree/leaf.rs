...
impl<'a> LeafNodeMut<'a> {
    ...
    /// Insert a cell at a known position (from a previous find_key NotFound result).
    /// This avoids the second find_key() call that insert_cell() would do.
    /// SAFETY: Caller must ensure insert_pos is valid (from find_key NotFound).
    pub fn insert_cell_at(&mut self, key: &[u8], value: &[u8], insert_pos: usize) -> Result<()> {
        let value_len_size = varint_len(value.len() as u64);
        let cell_size = key.len() + value_len_size + value.len();
        let space_needed = cell_size + SLOT_SIZE;

...
const TURDB_TARGET: &str =
    "/Users/julfikar/Documents/PassionFruit.nosync/turdb/turdb-core/.worktrees/update_bench_turdb";
...
fn benchmark_single_updates() {
    ...
    println!("\n============================================================");
    println!(
        "  Single Update Benchmark (Apple-to-Apple) - {} rows",
        ROW_COUNT
    );
    println!("============================================================\n");
    ...
    {
        let mut update_stmt = sqlite_conn
            .prepare_cached(
        ...
        for i in 0..ROW_COUNT {
            ...
            update_stmt
                .execute(rusqlite::params![new_title, new_version, (i % 1000) as i64])
    ...
    println!(
        "SQLite: {} updates in {:.3}s = {:.0} updates/sec\n",
        ROW_COUNT,
    ...
    turdb
        ...
        .expect("Failed to set PRAGMA WAL");
    turdb
        .execute("PRAGMA synchronous = OFF;")
        .expect("Failed to set PRAGMA synchronous");

    let update_stmt = turdb
        .prepare("UPDATE dataset_versions SET title = ?, version_number = ? WHERE dataset_id = ?")
    ...
    for i in 0..ROW_COUNT {
        ...
        let new_version = (i as f64) * 1.5;
        update_stmt
            .bind(OwnedValue::Text(new_title))
    ...
    println!(
        "TurDB:  {} updates in {:.3}s = {:.0} updates/sec\n",
        ROW_COUNT,
    ...
    for &i in &verification_indices {
        ...
        assert!(
            (actual_version - expected_version).abs() < 0.001,
    ...
    println!(
        "✓ Verified {} sample rows - all updates correct\n",
    ...
    println!("============================================================");
    println!(
        "SQLite (prepare_cached): {:.3}s ({:.0} updates/sec)",
        sqlite_elapsed.as_secs_f64(),
        ROW_COUNT as f64 / sqlite_elapsed.as_secs_f64()
    );
    println!(
        "TurDB (PreparedStatement): {:.3}s ({:.0} updates/sec)",
        turdb_elapsed.as_secs_f64(),
        ROW_COUNT as f64 / turdb_elapsed.as_secs_f64()
    );
...
fn benchmark_bulk_update() {
    ...
    println!("\n============================================================");
    println!(
        "  Bulk Update Benchmark (Apple-to-Apple) - {} rows",
        ROW_COUNT
    );
    println!("============================================================\n");
    ...
    let sqlite_elapsed = sqlite_start.elapsed();
    println!(
        "SQLite: {} rows updated in {:.3}s = {:.0} updates/sec\n",
        sqlite_updated,
        sqlite_elapsed.as_secs_f64(),
    ...
    let rows = turdb
        .query("SELECT COUNT(*) FROM dataset_versions WHERE title = 'Bulk Updated'")
        .expect("Count failed");
    let turdb_updated: i64 = rows[0].get_int(0).unwrap();

    println!(
        "TurDB:  {} rows updated in {:.3}s = {:.0} updates/sec\n",
        turdb_updated,
        turdb_elapsed.as_secs_f64(),
    ...
    println!("============================================================");
    println!(
        "SQLite: {:.3}s ({:.0} updates/sec)",
        sqlite_elapsed.as_secs_f64(),
        sqlite_updated as f64 / sqlite_elapsed.as_secs_f64()
    );
    println!(
        "TurDB:  {:.3}s ({:.0} updates/sec)",
        turdb_elapsed.as_secs_f64(),
        turdb_updated as f64 / turdb_elapsed.as_secs_f64()
    );
